...
* 当通过处于`S_OPEN`状态的session发送数据包给远端节点时：
    ...
    4. 如果`TS_ECHO`不等于`TS_ECHO_TX`，则将`TS_ECHO`的值设置为`TS_ECHO_TX`的值，设置`timestampEchoPresent`标记，设置`TS_EHCO_TX`的值为`timestampEcho`域的值。
...
在使用`S_OPEN`状态的session发送数据包给远端节点时，如果当前发送的时间戳不等于`TS_TX`，则将当前时间戳的值设置为`TS_TX`，设置数据包中的`timestampPresent`标记，将`TS_TX`的值设置为`timestamp`域的值。

在从`S_OPEN`状态的session中接收到来自远端节点的数据包时，如果数据包中设置了`timestampEchoPresent`标记，而且`timestampEcho`域的值不等于`TS_ECHO_RX`，则：

1. 设置`timestampEcho`的值为`TS_ECHO_RX`的值；
