...
当某个消息分片被废弃后，可能会在流的序号空间中留下断档。由于断档的存在，接收方为了重传丢失的分片，而将所有已接收到的消息都保存下来。`User Data`数据块（参见 [2.3.11][6]）中包含了`Forward Sequence Number(FSN)`数据块，以此来告知接收方，“分片序号小于等于`FSN`的分片不会被发送或重传。”

当接收方的已接收的序号空间中出现断档，而`SEND_QUEUE`中没有非废弃的消息分片的话，则发送方 *应该* 发送一个 `Forward Sequence Number Update(FSN Update)`数据块，其中包含了标记为废弃的、分片序号为`FSN`的、`fsnOffset`域为0的`User Data`数据块。`FSN Update`数据块使接收方可以跳过空档，将接收到的消息发给用户，而不是试图修复空档。`FSN Update`数据块可以被认为是传输或重传一个实际上没有发送数据的、已被废弃的分片序号。
...
流的接收方会在确认消息（参见 [2.3.13][7]和[2.3.14][8]）的`bufferBytesAvailable`域中声明它要出接收方接收的数据的总量。

如果`flow.F_OUTSTANDING_BYTES`域的值大于风雨从接收方接收到的缓冲区的生命值，则除非`flow.EXCEPTION == true`（参见 [3.6.2.3][13]），否则 *禁止* 流的发送方发送新数据到网络中。

<a name="3.6.2.9.1" />
#### 3.6.2.9.1 缓冲探测（Buffer Probe）

如果最近从接收方接收到的缓冲区声明中表明，缓冲区大小为0，而且流已经被接收方拒绝了，则发送方被挂起；即，当`RX_BUFFER_SIZE == 0 && EXCEPTION == false`时，发送方被挂起。为了防止由于丢失确认而导致重新打开接收窗口（receive window），被挂起流发送者 *应该* 不时的发送一个包含了`Buffer Probe`数据块（参见 [2.3.15][19]）的数据包。
...
每个接收流都包含了与该流相关的必要的上下文信息来接收从该流的接收方发送的消息，并将处理过的、完整的消息的呈现给用户。每个接收流的上下文信息至少包括：
...
因此，在下列情况中，流的接收方 *应该* 尽可能早的发送确认消息：
...
流接收方会维护一个缓冲区，在其中重装配、重排序将要传递给用户的消息（参见 [3.6.3.3][25]）。具体实现和用户可以限制流所使用的资源（包括缓冲区占用的内存）。
...
流的接收方可以声明一个关闭的接收窗口（closed receive window），即长度为0，以此来挂起发送流。

用户可以挂起接收流传递消息给用户的操作（参见 [3.6.3.3][25]）。这样会导致接收方的缓冲区被填满。

为了使在接收流被挂起时也能完成消息分片接收或修改断档，即使缓冲区中数据的总量超过了缓冲区的容量，流的接收方 *必须* 在确认消息（如果它没有被挂起的话）中至少声明一个缓冲块，除非缓冲区的容量是0；否则，会发生死锁，因为接收缓冲区会一直保持被填满的状态，并且无法释放空间，因为其中有空档而由于发送者被挂起而无法修复。
...
`Buffer Probe`数据块（参见 [2.3.15][19]）是由流的发送方发送（参见 [3.6.2.9.1][32]）的，用来查询当前的接收窗口声明（receive window advertisement）（以确认消息的形式）。
