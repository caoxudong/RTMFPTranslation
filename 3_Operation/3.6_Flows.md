...
发送者发送用户消息分片（包括用户元数据和`Return Flow Association`（如果存在的话））给接收者标志着流的开始，此后发送者发送的消息分片中也一直会包含用户元数据和`Return Flow Association`（如果存在的话），直到接收者确认了这个流的建立，此时发送者孩子到接收到已经获取到了用户元数据和`Return Flow Association`（如果存在的话）。此时，流标识符（flow identifier）建立完成。
